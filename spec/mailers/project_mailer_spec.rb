require "rails_helper"

RSpec.describe ProjectMailer, type: :mailer do
  describe "project_created" do
    let(:mail) { ProjectMailer.project_created }

    it "renders the headers" do
      expect(mail.subject).to eq("Project created")
      expect(mail.to).to eq(["to@example.org"])
      expect(mail.from).to eq(["from@example.com"])
    end

    it "renders the body" do
      expect(mail.body.encoded).to match("Hi")
    end
  end

  describe "project_updated" do
    let(:mail) { ProjectMailer.project_updated }

    it "renders the headers" do
      expect(mail.subject).to eq("Project updated")
      expect(mail.to).to eq(["to@example.org"])
      expect(mail.from).to eq(["from@example.com"])
    end

    it "renders the body" do
      expect(mail.body.encoded).to match("Hi")
    end
  end

  describe "deadline_reminder" do
    let(:mail) { ProjectMailer.deadline_reminder }

    it "renders the headers" do
      expect(mail.subject).to eq("Deadline reminder")
      expect(mail.to).to eq(["to@example.org"])
      expect(mail.from).to eq(["from@example.com"])
    end

    it "renders the body" do
      expect(mail.body.encoded).to match("Hi")
    end
  end

end
